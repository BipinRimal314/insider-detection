...
% ============================================================================
\begin{abstract}
When detecting insider threats with unsupervised anomaly detection, the most widely used evaluation metric---AUC-ROC---gives a misleading answer. On the CMU-CERT r4.2 dataset, Isolation Forest achieves the highest AUC-ROC (0.799), suggesting it is the best model. But at the 5\% false positive rate where security teams actually operate, an LSTM autoencoder catches \textbf{3.4$\times$ more attacks} (Recall@5\%FPR: 0.149 vs.\ 0.044, $p = 0.031$, one-sided Wilcoxon).
...
\begin{enumerate}
    \item Evidence that temporal modeling (LSTM autoencoder) achieves 3.4$\times$ higher detection at operational thresholds despite lower AUC-ROC, with statistical significance ($p = 0.031$, one-sided Wilcoxon).
...
\midrule
Isolation Forest & $\mathbf{0.799 \pm .017}$ & $0.044 \pm .009$ & $0.220 \pm .025$ \\
PCA Recon. & $0.612 \pm .000$ & $0.049 \pm .000$ & $0.129 \pm .000$ \\
Dense AE & $0.659 \pm .016$ & $0.048 \pm .006$ & $0.118 \pm .009$ \\
LSTM AE & $0.770 \pm .006$ & $\mathbf{0.149 \pm .021}$ & $\mathbf{0.254 \pm .019}$ \\
\bottomrule
\end{tabular}
\end{table}

Isolation Forest is the best \textit{ranker}---it orders anomalies most accurately across the full score distribution (AUC-ROC = 0.799). But at the 5\% FPR threshold, it detects only 4.4\% of insider days. The LSTM autoencoder detects 14.9\%---a \textbf{3.4$\times$ improvement} ($p = 0.031$, one-sided Wilcoxon signed-rank).
...
\caption{The reversal. Left: AUC-ROC favors Isolation Forest. Right: Recall@5\%FPR favors the LSTM autoencoder by 3.4$\times$. The ``best'' model depends entirely on which metric you trust.}
...
\midrule
7 days & $\mathbf{0.770 \pm .006}$ & $\mathbf{0.149 \pm .021}$ & $\mathbf{0.254 \pm .019}$ \\
14 days & $0.765 \pm .008$ & $0.142 \pm .018$ & $0.248 \pm .022$ \\
30 days & $0.752 \pm .012$ & $0.128 \pm .025$ & $0.235 \pm .028$ \\
\bottomrule
\end{tabular}
\end{table}

Here, unlike the model comparison, both metrics agree: the 7-day window is best on every measure. AUC-ROC declines monotonically with window length ($0.770 \to 0.765 \to 0.752$), and Recall@5\%FPR---the operationally relevant metric---declines in step ($0.149 \to 0.142 \to 0.128$). Longer windows do not help; they hurt, and they hurt on both the ranking metric and the operational one.

Why? Lengthening the window dilutes attack signal. A 7-day sequence in which an attack occupies one or two days is already dominated by normal behavior; stretching the window to 14 or 30 days buries the attack further, spreading a fixed amount of anomalous activity across proportionally more normal context. The reconstruction error the LSTM must flag is averaged over a longer, mostly-benign sequence, so both global ranking and low-FPR recall degrade. This validates the 7-day window used throughout the main results: it is short enough to keep attack behavior a substantial fraction of each sequence while still spanning a full weekly behavioral cycle. Note that this within-model ablation does not contradict the cross-model reversal of Section~\ref{sec:results}---there, AUC-ROC and Recall@5\%FPR disagree about \emph{which model} is best; here, holding the model fixed, they agree about \emph{which window} is best.
...
These findings suggest a practical deployment: use Isolation Forest for rapid, low-cost screening of all users (its high AUC-ROC provides effective prioritization), then apply the LSTM autoencoder to the flagged subset for temporal analysis. The 3.4$\times$ detection improvement at low FPR justifies the additional computational cost for high-risk users.
...
The standard evaluation metric for anomaly detection---AUC-ROC---gives a misleading picture of insider threat detection performance. On the CMU-CERT r4.2 dataset, Isolation Forest achieves the highest AUC-ROC (0.799 $\pm$ 0.017), but the LSTM autoencoder provides 3.4$\times$ higher recall at the 5\% FPR threshold where security teams operate (0.149 vs.\ 0.044, $p = 0.031$).
...
Three additional findings bear emphasis. First, USB device activity is the strongest attack indicator, suggesting that removable media monitoring should be prioritized in deployment. Second, the 7-day window is optimal on every metric we measured (AUC-ROC 0.770, Recall@5\%FPR 0.149); extending the window to 14 or 30 days dilutes attack signal and degrades both ranking and operational recall, validating the sequence length used throughout. Third, and most soberly, 86\% of attack sequences evade detection even under the best model. The ``boiling frog'' pattern---insiders who pace their activity to stay within normal bounds---represents a fundamental challenge for any system that defines threats as deviations from a learned baseline.
