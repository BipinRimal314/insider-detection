...
**Current state:** Source code tested (33/33), 4 experiments run on agent datasets (TRAIL + TRACE), results and figures generated. Next: run full pipeline including CMU-CERT on Ubuntu PC.
...
### Source Code (all tested, 33/33 passing)
...
### Experiments (agent datasets only — CMU-CERT pending on PC)
- **Experiment 1 — Baselines:** TRAIL DC=0.921, LSTM=0.744, IF=0.597. TRACE ~0.46 (reward hacking undetectable)
- **Experiment 2 — Transfer:** TRACE→TRAIL IF improves +0.208 (richer normal baseline). TRAIL→TRACE DC drops -0.431
- **Experiment 3 — OWASP:** ASI01=0.836, ASI02=0.380 (blind spot), ASI05=0.834, ASI09=0.885, ASI10=0.889
- **Experiment 4 — Governance:** KL divergence analysis, governance report generated
...
## Still TODO (on Ubuntu PC)

- [ ] Set up Ubuntu environment with CUDA
- [ ] Download CMU-CERT dataset to PC (large, needs SSD space)
- [ ] Re-run Experiment 1 with CMU-CERT baseline (reproduce 0.985 AUC-ROC)
- [ ] Re-run Experiment 2 with CMU-CERT ↔ Agent transfer (the core contribution)
- [ ] Multi-seed experiments (seeds 42-46) for confidence intervals
- [ ] Re-generate figures with CMU-CERT data included
